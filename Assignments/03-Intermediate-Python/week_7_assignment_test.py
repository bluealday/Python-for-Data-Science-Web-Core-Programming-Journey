...
# check the white pages filter functionality
#city_state_dataframe = my_white_pages_parser.pages_tbl_filter_by(address_city="oRlandO", address_state="fL")
city_state_dataframe = my_white_pages_parser.pages_tbl_filter_by()
my_white_pages_parser.to_csv(city_state_dataframe, "./course_2_intermediate_python/week_7/assignments/output_data/white_pages_city_orlando_state_fl.csv")  

# Look for employees with the title "Software Engineer" in the company pages.  Write results to csv files:
title_software_engineer_dataframe = my_company_pages_parser.pages_tbl_filter_by(title="software engineer")
my_company_pages_parser.to_csv(title_software_engineer_dataframe, "./course_2_intermediate_python/week_7/assignments/output_data/company_pages_titles_software_engineer.csv")  
...
# Look for attorney businesses with no websites in zip 32501 or aread code 954. Write result to csv file. 
# This lookup was done in a prior week and it took to operations.  
# Note how now this can be done in a single operation after we've modified our classes to use Pandas DataFrame:
